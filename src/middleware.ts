...
export const onRequest = defineMiddleware(async (context, next) => {
  const pathname = context.url.pathname; const adminSurface = pathname.startsWith('/admin') || pathname.startsWith('/api/admin');
  if (!adminSurface) return next();
  let sessionValid = false; let sessionReason = 'missing'; let bootstrapRequired = false;
  try {
    const config = loadAuthConfig(); const credentials = await loadAdminCredentials(createNetlifyAdminStore()); const verified = await verifySession(context.cookies.get(SESSION_COOKIE)?.value, config, credentials);
    sessionValid = verified.valid; sessionReason = ('reason' in verified && verified.reason) ? verified.reason : 'valid'; bootstrapRequired = Boolean(verified.valid && verified.user?.bootstrapRequired);
    if ('user' in verified && verified.user) context.locals.adminUser = verified.user;
  } catch { sessionReason = 'configuration'; }
  const decision = accessDecision(pathname, sessionValid, bootstrapRequired);
  ...
  if (decision.action === 'redirect-change-password') return context.redirect(new URL('/admin/alterar-senha', context.url).toString(), 302);
  if (decision.action === 'json-bootstrap-required') return jsonResponse(apiError('BOOTSTRAP_REQUIRED', 'Defina uma nova senha para continuar.'), 403, securityHeaders);
