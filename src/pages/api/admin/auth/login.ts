...
import { authenticateAdmin } from '../../../../lib/admin/auth-service.mjs';
...
export const POST: APIRoute = async ({ request, cookies, url }) => {
  try {
    ...
    const config = loadAuthConfig(); const auth = await authenticateAdmin(createNetlifyAdminStore(), username.trim(), password);
    if (!auth.valid) return jsonResponse(apiError('INVALID_CREDENTIALS', 'Usuário ou senha inválidos.'), 401);
    const session = await createSession(config, auth.credentials); cookies.set(SESSION_COOKIE, session.token, sessionCookieOptions(config.ttl) as Parameters<typeof cookies.set>[2]);
    return jsonResponse(apiPayload({ user: { username: auth.credentials.username, role: config.role }, expiresAt: session.payload.exp, bootstrapRequired: auth.credentials.bootstrap_required }));
