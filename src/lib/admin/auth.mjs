...
export async function verifyCredentials(username, password, credentials) {
  const [, nText, rText, pText, saltText, expectedText] = credentials.passwordHash.split('$'); const expected = Buffer.from(expectedText, 'base64url'); const derived = await scrypt(String(password), Buffer.from(saltText, 'base64url'), expected.length, { N: Number(nText), r: Number(rText), p: Number(pText), maxmem: 64 * 1024 * 1024 });
