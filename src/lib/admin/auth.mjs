...
async function derivePasswordHash(password, { N = 16384, r = 8, p = 1 } = {}) {
...
export async function hashPassword(password, options) {
  ...
  return derivePasswordHash(password, options);
}
export function validateNewPassword(password, confirmation, username) {
  ...
  if (safeTextEqual(password.toLocaleLowerCase('pt-BR'), 'admin')) return { valid: false, code: 'PASSWORD_NOT_ALLOWED', message: 'Escolha uma senha diferente da credencial temporária.' };
...
export function hashBootstrapPassword() { return derivePasswordHash('admin'); }
