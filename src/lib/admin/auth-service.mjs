...
export async function authenticateAdmin(store, username, password, loadOptions) {
  const credentials = await loadAdminCredentials(store, loadOptions); const valid = await verifyCredentials(username, password, credentials); return { valid, credentials };
}
export async function changeAdminPassword(store, currentPassword, newPassword, confirmation, role = 'administrador', loadOptions) {
  const current = await loadAdminCredentials(store, loadOptions);
  ...
  const passwordHash = await hashPassword(newPassword); const credentials = await persistDefinitivePassword(store, current, passwordHash); return { ok: true, credentials };
}