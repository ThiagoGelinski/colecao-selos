...
export function getAdminConfigurationStatus(env = process.env) { return { site_url_configurada: Boolean(env.SITE_URL), autenticacao_configurada: Boolean(env.ADMIN_SESSION_SECRET), persistencia_credenciais: '@netlify/blobs', bootstrap_seguro: true, role_padrao: env.ADMIN_ROLE || 'administrador', duracao_sessao_segundos: Number(env.ADMIN_SESSION_TTL_SECONDS || 28800) }; }
