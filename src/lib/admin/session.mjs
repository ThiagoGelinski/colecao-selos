...
export async function createSession(config, credentials, now = Date.now()) {
  const payload = { sub: credentials.username, role: config.role, cv: credentials.credential_version, bootstrap: credentials.bootstrap_required, iat: Math.floor(now / 1000), exp: Math.floor(now / 1000) + config.ttl, nonce: crypto.randomUUID() };
...
export async function verifySession(token, config, credentials, now = Date.now()) {
  ...
  try {
    ...
    const payload = JSON.parse(new TextDecoder().decode(fromBase64Url(encoded))); if (payload.sub !== credentials.username || payload.cv !== credentials.credential_version || payload.bootstrap !== credentials.bootstrap_required || !ADMIN_ROLES.includes(payload.role)) return { valid: false, reason: 'invalid' };
    if (!Number.isInteger(payload.exp) || payload.exp <= Math.floor(now / 1000)) return { valid: false, reason: 'expired' };
    return { valid: true, user: { username: payload.sub, role: payload.role, bootstrapRequired: payload.bootstrap }, expiresAt: payload.exp };
