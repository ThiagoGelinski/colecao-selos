...
const DEFAULT_TTL_SECONDS = 60 * 60 * 8; const MIN_BOOTSTRAP_PASSWORD_LENGTH = 16;
...
export function loadBootstrapPassword(env = process.env) {
  const password = env.ADMIN_BOOTSTRAP_PASSWORD;
  if (typeof password !== 'string' || password.trim().length === 0) throw new AuthConfigurationError('Segredo de bootstrap administrativo não configurado.');
  if (password.length < MIN_BOOTSTRAP_PASSWORD_LENGTH) throw new AuthConfigurationError(`ADMIN_BOOTSTRAP_PASSWORD deve ter ao menos ${MIN_BOOTSTRAP_PASSWORD_LENGTH} caracteres.`);
  return password;
}
