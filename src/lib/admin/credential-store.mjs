...
import { hashBootstrapSecret } from './auth.mjs';
import { loadBootstrapConfig } from './config.mjs';
import { logAdminAuth } from './logging.mjs';
const STORE_NAME = 'colecao-selos-admin'; const CREDENTIALS_KEY = 'auth/credentials-v1'; const STATE_KEY = 'auth/bootstrap-state-v1'; const BOOTSTRAP_MODE_VERSION = 3;
...
async function initialCredentials(timestamp, bootstrapSecret, version = 1) { return { schema_version: 1, username: 'admin', password_hash: await hashBootstrapSecret(bootstrapSecret), bootstrap_required: true, bootstrap_consumed: false, credential_version: Math.max(1, version), updated_at: timestamp }; }
...
async function repairPartialState(store, credentials, state, now, bootstrapEnv) {
  const timestamp = now.toISOString();
  if (!credentials && !state) {
    const { bootstrapSecret } = loadBootstrapConfig(bootstrapEnv);
    logAdminAuth('bootstrap_initialization', { has_credentials: false, has_state: false }); const credentialResult = await store.createCredentials(await initialCredentials(timestamp, bootstrapSecret)); if (credentialResult.modified) await store.createBootstrapState(initialState(timestamp)); return readPair(store);
  }
  if (!credentials && state) {
    ...
    const { bootstrapSecret } = loadBootstrapConfig(bootstrapEnv);
    logAdminAuth('partial_state_repair', { has_credentials: false, has_state: true, repair: 'create_credentials' }); await store.createCredentials(await initialCredentials(timestamp, bootstrapSecret)); return readPair(store);
  }
  if (credentials && !state) {
...
async function normalizePersistedState(store, credentials, state, now, bootstrapEnv) {
  ...
  const { bootstrapSecret } = loadBootstrapConfig(bootstrapEnv);
  logAdminAuth('legacy_migration', { bootstrap_consumed: false, bootstrap_required: credentials.value?.bootstrap_required ?? null, repair: 'reset_unconsumed_bootstrap' }); return replacePair(store, credentials, state, await initialCredentials(timestamp, bootstrapSecret, currentVersion + 1), { ...initialState(timestamp), ...state.value, schema_version: 1, initialized: true, bootstrap_consumed: false, bootstrap_mode_version: BOOTSTRAP_MODE_VERSION, updated_at: timestamp });
}
export async function loadAdminCredentials(store = createNetlifyAdminStore(), { now = new Date(), bootstrapEnv = process.env } = {}) {
  try {
    let [credentials, state] = await readPair(store);
    for (let attempt = 0; attempt < 3 && (!credentials || !state); attempt += 1) { [credentials, state] = await repairPartialState(store, credentials, state, now, bootstrapEnv); }
