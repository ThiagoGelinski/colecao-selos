...
export function safeApiFailure(error) {
  if (error?.name === 'AuthConfigurationError' || error?.name === 'AdminStorageError') return jsonResponse(apiError('AUTH_UNAVAILABLE', 'Autenticação administrativa indisponível.'), 503);
