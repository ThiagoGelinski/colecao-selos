...
A área administrativa server-side está documentada em [docs/admin/README.md](docs/admin/README.md). Ela usa rotas server-side protegidas no Netlify, reaproveita os registros e a validação operacional completa da pipeline, incluindo assets. O painel permite cadastro, upload inicial e retificação controlada de imagens, mas não concede aprovação nem altera a política de publicação. O primeiro acesso administrativo usa o usuário `admin` e um segredo de ativação de uso único fornecido exclusivamente pelo ambiente server-side. O hash é persistido no Netlify Blobs e `bootstrap_consumed=true` impede a reativação. Configure `ADMIN_SESSION_SECRET`, `ADMIN_BOOTSTRAP_ENABLED` e `ADMIN_BOOTSTRAP_SECRET` conforme `docs/admin/README.md`; depois da ativação definitiva, desabilite e remova o segredo de bootstrap. Nunca versione credenciais reais.
